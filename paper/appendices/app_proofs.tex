...
main results rest. We recall the standing notation: $d$ is the residual stream dimension,
...
% ==============================================================================
\subsection{Proof of Proposition: Metric Preservation of Attention Transport}
...
\end{align*}
By the orthogonality of the subspaces (Assumption~\ref{ass:orthogonality}),
...
\begin{proof}
The curvature tensor (Definition~\ref{def:curvature}) is defined by
\[
...
the expected direct transport from $x_i$ to $x_k$. Geometrically, the parallel transport
...
\end{align*}
Now observe that $g_k^{(l)}\, I_{d_l}\, (g_k^{(l)})^{-1} = I_{d_l}$, since the identity
matrix commutes with every invertible matrix. Therefore:
\begin{align*}
    g_k^{(l)}\, [F_{ijk}]_l\, (g_k^{(l)})^{-1} - I_{d_l}
    &= g_k^{(l)}\, [F_{ijk}]_l\, (g_k^{(l)})^{-1}
       - g_k^{(l)}\, I_{d_l}\, (g_k^{(l)})^{-1} \\
    &= g_k^{(l)}\, \big([F_{ijk}]_l - I_{d_l}\big)\, (g_k^{(l)})^{-1}.
\end{align*}
Applying identity~\eqref{eq:frob-invariance} with $Q = g_k^{(l)}$ and $A = [F_{ijk}]_l -
I_{d_l}$:
\begin{align*}
    \big\|[F_{ijk}^{\,\mathrm{gauge}}]_l - I_{d_l}\big\|_F
    &= \big\|g_k^{(l)}\, \big([F_{ijk}]_l - I_{d_l}\big)\,
       (g_k^{(l)})^{-1}\big\|_F \\
    &= \big\|[F_{ijk}]_l - I_{d_l}\big\|_F.
\end{align*}
...
here ensures that the diagnostic framework of Corollary~\ref{cor:binding-failure}---which
...
\begin{proposition}[Block Decomposition of Total Curvature, restated from
...
block of $F_{ijk}$, and $P_{W_k} = I_{d_k}$ (as an operator on $W_k$).
...
Assumption~\ref{ass:orthogonality} and the block-diagonal structure of $U_{ij}$
(Proposition~\ref{prop:block-structure}), the cross-layer terms vanish at the single-layer
level. However, as shown in Proposition~\ref{prop:cross-term}, multi-layer composition can
introduce non-zero off-diagonal curvature, reflecting the non-abelian coupling between
standpoint layers.
\end{proof}
...
\begin{enumerate}
    ...
    parallel transport $U_{ij}$.
    \item The commitment in layer $k$ at event $x_i$ is encoded in $\psi_k(x_i) =
    P_{W_k}\, h_{x_i}$, the projection of the residual stream onto $W_k$.
...
(Corollary~\ref{cor:binding-failure}):
\[
    \|F_{ijk}\|_k = \big\|[F_{ijk}]_k - I_{d_k}\big\|_F > 0,
\]
