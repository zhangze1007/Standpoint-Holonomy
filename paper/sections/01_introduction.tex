...
individual failure scenarios (T2--T5) are not yet available. This T0 anomaly suggests
that the most geometrically complex internal states arise from unconstrained knowledge
retrieval rather than external pressure. Third, block-specific holonomy concentration
(H1) is not confirmed: the five standpoint blocks are highly correlated ($r > 0.97$),
and principal component analysis shows that the first component accounts for $>99.99\%$
of variance with approximately uniform loadings, indicating a dominant global coherence
signal rather than five independent dimensions. The five-layer decomposition is
accordingly interpreted as an interpretable diagnostic projection coordinate system, not
as a decomposition into causally independent cognitive processes.
