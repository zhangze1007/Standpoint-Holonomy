...
defines a discrete connection on a fiber bundle. The key difference from prior
work is that we do not merely \emph{describe} what heads do --- we prove that
their collective operation defines a connection, with curvature measuring
the inter-layer inconsistency and standpoint drift.
...
a DAG, with the additional structure that disagreement can be projected onto
specific standpoint layers via the block decomposition of $F_{ijk}$ (though the
dominant signal is global; see Section~\ref{sec:reframing}).
...
Linear probing~\cite{belinkov2021probing} trains a classifier on a layer's
activations to predict whether a specific property is encoded. This answers
\emph{what information a layer contains}. Holonomy deviation answers a
complementary question: \emph{whether that information is preserved across
events}. A layer can have high probing accuracy (it encodes the right
commitment) yet high holonomy deviation (it fails to transport that commitment
consistently). The two diagnostics are orthogonal: probing measures
representational content, holonomy deviation measures transport fidelity. A
complete diagnostic requires both.
