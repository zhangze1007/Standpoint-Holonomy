...
These hypotheses require additional GPU resources for activation extraction and
permutation testing. The non-reporting of H8.19--H8.21 is a computational
...
\textbf{Status} \\
\midrule
H1a & Block sensitivity & Above-mean ratio & T2: 73\%, T4: 83\%,
T3/T5: FAIL & T3/T4/T5: 100\%,
T2: 0\% & Partial \\
...
$^\dagger$Llama-7b ablation requires additional GPU resources.}
\end{table}
...
\midrule
\multirow{6}{*}{GPT-2}
& T0 & 30 & $18.44$ & $6.94$ & $12.03$ \\
& T1 & 30 & $12.97$ & $6.71$ & $4.92$ \\
& T2 & 30 & $22.22$ & $11.75$ & $16.38$ \\
& T3 & 30 & $19.55$ & $4.07$ & $1.65$ \\
& T4 & 30 & $19.72$ & $8.66$ & $11.46$ \\
& T5 & 30 & $31.42$ & $12.16$ & $20.66$ \\
\midrule
\multirow{6}{*}{Llama-7b}
...
\medskip
{\footnotesize SD = standard deviation; IQR = interquartile range (Q3$-$Q1).
GPT-2 shows higher within-scenario variance than Llama-7b, with T5 having the
highest median ($31.42$) in GPT-2 versus T0 ($47.52$) in Llama-7b. The T0 anomaly
(T0 $>$ T1) is present in both models but with different magnitude.}
\end{table}
...
\begin{remark}[T0 Surface Separability]
\label{rem:t0-separability}
The T0 anomaly (T0 producing higher holonomy than all failure scenarios) could
reflect surface-level differences in the stimuli rather than genuine geometric
complexity. We test this by regressing global holonomy on scenario type and
surface features (conversation length, lexical overlap between consecutive
assistant turns, entity count):
\[
    C = \beta_0 + \beta_1 \cdot \text{Scenario} + \beta_2 \cdot \text{Length}
    + \beta_3 \cdot \text{LexOverlap} + \beta_4 \cdot \text{Entities} + \varepsilon.
\]
Surface features alone explain negligible variance ($R^2 = 0.016$, $p = 0.70$).
After controlling for surface features, scenario type retains a partial $R^2 =
0.194$, and the T0 coefficient remains significant ($\beta = 6.01$, $p = 0.0007$).
This confirms that the T0 anomaly is not an artifact of stimulus surface properties.
\end{remark}
