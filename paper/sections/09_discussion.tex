...
the diagnostic power degrades. Null grouping controls (random, shuffled, layer-uniform)
are needed to verify that the learned grouping provides non-trivial diagnostic value.
...
reference-relative rather than intrinsic. Different baseline selections yield different
holonomy values. Baseline ensemble analysis (estimating $U_{ik}^{\exp}$ from multiple
random subsamples of T1) is needed to verify that empirical conclusions are
baseline-robust.
...
\|F\|_{\mathrm{pos}})$ across all 180 Llama-7b conversations reveals that the first
...
two diagnostics are orthogonal: a layer can have high probing accuracy (it encodes the
