...
\section{Mathematical Instantiation: Transformer Attention as Discrete Connection}
\label{sec:instantiation}

This section establishes the central mathematical result of the paper: a transformer's
attention mechanism defines a discrete connection on a fiber bundle over the event-sequence
space, and its curvature \emph{is} the path-dependence that measures standpoint
drift~\cite{LCESA} and binding failure~\cite{self-jurisdiction}. Unlike classical parallel
transport, this connection is not necessarily metric-compatible; the curvature measures the
\emph{differential} transport across standpoint layers, which is the geometrically
meaningful quantity. We proceed by instantiating each component of the abstract
...
different standpoint layers. The curvature of this connection, defined in
...
\begin{proposition}[Curvature as Inter-Layer Inconsistency]
\label{prop:curvature-inconsistency}
Under the block-diagonal structure ($\delta = 0$), the block-specific curvature norm has
the explicit form
\[
    \|F_{ijk}\|_k = \left|\frac{\bar{\alpha}_{ji}^{(k)}\, \bar{\alpha}_{kj}^{(k)}}
    {\bar{\alpha}^{(k)}_{\exp}} - 1\right| \cdot \sqrt{d_k}.
\]
The curvature is zero if and only if the composite attention weight for layer $k$ matches
the baseline expectation. The curvature \emph{across} layers measures the inconsistency of
...
\begin{definition}[Block-Specific Curvature]
...
\begin{theorem}[Block-Specific Curvature Detects Binding Failure]
...
$\delta = 0$), $U_{ij} = \sum_k \bar{\alpha}_{ji}^{(k)} P_{W_k}$ and the curvature
...
For the non-orthogonal case ($\delta > 0$), the block-specific curvature acquires an
...
The gauge-transformed curvature is $F^g_{ijk} = g_k\, F_{ijk}\, g_k^{-1}$. The
block-specific curvature transforms as:
\begin{align*}
    [F^g_{ijk}]_k
...
Note: this proof does \emph{not} require the subspaces to be orthogonal or the curvature
to be block-diagonal. The gauge invariance of block-specific curvature norms holds for
